...
var dgram           = require('node-dgram-dtls')
  , net             = require('net')
  , util            = require('util')
...
CoAPServer.prototype.listen = function(port, address, done) {
  ...
  if (this._options.dtls && this._options.dtls==true)
  {
    this._sock.enableDTLS(true);
    if (this._options.privateKey)
    {
      this._sock.setPrivateKey(this._options.privateKey);
    }
    if (this._options.certificate)
    {
      this._sock.setCertificate(this._options.certificate);
    }
  }
